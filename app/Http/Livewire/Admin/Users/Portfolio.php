...
class Portfolio extends Component
{
    ...
    {
        $this->followers = Followed::whereUserId($this->client->id)
            ->when($this->search, function ($query) {
                $query->where(function ($query) {
                    $query->WhereRelation('user', 'first_name', 'like', '%' . $this->search . '%')
                        ->orWhereRelation('plan', 'name', 'like', '%' . $this->search . '%')
                        ->orWhereRelation('plan', 'details', 'like', '%' . $this->search . '%')
                        ->orWhereRelation('user', 'last_name', 'like', '%' . $this->search . '%');
                });
            })
            ->orderby($this->orderBy, $this->sortBy)
            ->paginate($this->perPage);
        return view('livewire.admin.users.portfolio', ['followers' => $this->followers]);
    }
}
