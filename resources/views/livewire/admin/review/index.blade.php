        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">{{__('Filter Reviews')}}</h3>
            ...
            <div class="row g-xl-8">
                <div class="col-md-8">
                    <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                        <div class="input-group input-group-solid mb-5 rounded-4">
                            ...
                            <input type="search" class="form-control form-control-solid text-dark" wire:model="search" placeholder="{{__('Search reviews')}}" />
                        </div>
                    </div>
                </div>
                <div class="col-md-4 text-end">
                    ...
                    <button id="kt_addreview_button" class="btn btn-dark me-4"><i class="fal fa-plus"></i> {{__('Add review')}}</button>
                </div>
            </div>
            <div wire:ignore.self id="kt_addreview" class="bg-white" data-kt-drawer="true" data-kt-drawer-activate="true" data-kt-drawer-toggle="#kt_addreview_button" data-kt-drawer-close="#kt_addreview_close" data-kt-drawer-width="{'md': '900px'}">
                <div class="card w-100">
                    <div class="card-header pe-5 border-0">
                        <div class="card-title">
                            <div class="d-flex justify-content-center flex-column me-3">
                                <div class="fs-4 fw-bolder text-gray-900 text-hover-primary me-1 lh-1">{{__('Add a Review')}}</div>
                            </div>
                        </div>
                        <div class="card-toolbar">
                            <div class="btn btn-sm btn-icon btn-icon-dark btn-active-light-primary" data-kt-drawer-dismiss="true" id="kt_addreview_close">
                    ...
                    <div class="card-body text-wrap">
                        <div class="pb-5 mt-10 position-relative zindex-1">
                            <form class="form w-100 mb-10" wire:submit.prevent="addReview" method="post">
                                <div class="row">
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <div class="fv-row mb-6">
                                            <input class="form-control form-control-lg form-control-solid" type="text" wire:model.defer="occupation" required placeholder="Occupation" />
                                            @error('occupation')
                                            <span class="form-text text-danger">{{$message}}</span>
                                            @enderror
                                        </div>
                                        <div class="fv-row mb-6">
                                            <textarea class="form-control form-control-lg form-control-solid" rows="10" type="text" wire:model.defer="review" required placeholder="Occupation"></textarea>
                                            @error('review')
                                            <span class="form-text text-danger">{{$message}}</span>
                                            @enderror
                                        </div>
                                        <div class="text-center mt-10">
                                            <button type="submit" class="btn btn-lg btn-info btn-block fw-bolder me-3 my-2">
                                                <span wire:loading.remove wire:target="addReview">{{__('Submit Review')}}</span>
                                                <span wire:loading wire:target="addReview">{{__('Processing Request...')}}</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="table-responsive">
                <table id="kt_datatable_zero_configuration" class="table table-row-bordered gy-5" wire:loading.class.delay="opacity-50" wire:target="search, status, sortBy, orderBy, perPage, loadMore">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="min-w-100px">{{__('Occupation')}}</th>
                ...
                @if($reviews->total() > 0 && $reviews->count() < $reviews->total())<button wire:click="loadMore" wire:loading.remove class="btn btn-secondary btn-block">{{__('See more')}}</button>@endif
            </div>
            @else
            <div class="text-center mt-20">
                <img src="{{asset('asset/images/beneficiary.png')}}" style="height:auto; max-width:250px;" class="mb-6">
                <h3 class="text-dark">{{__('No Review Found')}}</h3>
                <p class="text-dark">{{__('We couldn\'t find any reviews')}}</p>
            </div>
            @endif

    @foreach($reviews as $val)
    <livewire:admin.review.edit :val=$val :wire:key="'kt_edit_'. $val->id">
        @endforeach
</div>