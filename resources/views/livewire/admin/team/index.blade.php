        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">{{__('Filter Team')}}</h3>
            ...
            <div class="row g-xl-8">
                <div class="col-md-8">
                    <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                        <div class="input-group input-group-solid mb-5 rounded-4">
                            ...
                            <input type="search" class="form-control form-control-solid text-dark" wire:model="search" placeholder="{{__('Search teams')}}" />
                        </div>
                    </div>
                </div>
                <div class="col-md-4 text-end">
                    ...
                    <button id="kt_addteam_button" class="btn btn-dark me-4"><i class="fal fa-plus"></i> {{__('Add team')}}</button>
                </div>
            </div>
                    ...
                    <div class="card-header pe-5 border-0">
                        <div class="card-title">
                            <div class="d-flex justify-content-center flex-column me-3">
                                <div class="fs-4 fw-bolder text-gray-900 text-hover-primary me-1 lh-1">{{__('Add a Team')}}</div>
                            </div>
                        </div>
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <div class="text-center mt-10">
                                            <button type="submit" class="btn btn-lg btn-info btn-block fw-bolder me-3 my-2">
                                                <span wire:loading.remove wire:target="addTeam">{{__('Submit Team')}}</span>
                                                <span wire:loading wire:target="addTeam">{{__('Processing Request...')}}</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="table-responsive">
                <table id="kt_datatable_zero_configuration" class="table table-row-bordered gy-5" wire:loading.class.delay="opacity-50" wire:target="search, status, sortBy, orderBy, perPage, loadMore">
                    <thead>
                        <tr class="fw-semibold fs-6 text-muted">
                            ...
                            <th class="min-w-100px">{{__('Position')}}</th>
                ...
                @if($teams->total() > 0 && $teams->count() < $teams->total())<button wire:click="loadMore" wire:loading.remove class="btn btn-secondary btn-block">{{__('See more')}}</button>@endif
            </div>
            @else
            <div class="text-center mt-20">
                <img src="{{asset('asset/images/beneficiary.png')}}" style="height:auto; max-width:250px;" class="mb-6">
                <h3 class="text-dark">{{__('No Team Found')}}</h3>
                <p class="text-dark">{{__('We couldn\'t find any teams')}}</p>
            </div>
            @endif

    @foreach($teams as $val)
    <livewire:admin.team.edit :val=$val :wire:key="'kt_edit_'. $val->id">
        @endforeach
</div>