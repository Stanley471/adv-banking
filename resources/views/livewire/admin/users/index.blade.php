<div>
    <div class="toolbar" id="kt_toolbar">
        <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
            <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                <h1 class="text-dark fw-bolder my-1 fs-2">{{__('Clients')}}</h1>
            ...
            <div class="d-flex align-items-center flex-nowrap text-nowrap py-1">
                ...
                <button id="kt_user_button" class="btn btn-dark me-4"><i class="fal fa-plus"></i> {{__('Add a User')}}</button>
            </div>
        </div>
    </div>
            ...
            <div class="card-header pe-5 border-0">
                <div class="card-title">
                    <div class="d-flex justify-content-center flex-column me-3">
                        <div class="fs-4 fw-bolder text-gray-900 text-hover-primary me-1 lh-1">{{__('Create a User')}}</div>
                    </div>
                </div>
                ...
                <div class="pb-5 mt-10 position-relative zindex-1">
                    <form class="form w-100 mb-10" wire:submit.prevent="addUser" method="post">
                        ...
                        <div class="fv-row mb-6" wire:ignore>
                            <label class="form-label fs-6 fw-bolder text-dark">{{__('Phone Number')}}</label>
                        ...
                        <div class="text-center mt-10">
                            <button type="submit" class="btn btn-lg btn-info btn-block fw-bolder me-3 my-2">
                                ...
                                <span wire:loading wire:target="addUser">{{__('Processing Request...')}}</span>
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="fv-row mb-6">
                        <label class="form-label fs-6 fw-bolder text-dark">{{__('Email verified')}}</label>
                    ...
                    <div class="fv-row mb-6">
                        <label class="form-label fs-6 fw-bolder text-dark">{{__('Phone verified')}}</label>
            ...
            <div class="row g-xl-8">
                <div class="col-lg-12 col-md-12">
                    <div class="d-flex justify-content-center flex-column me-3">
                        <div class="col-md-12">
                            <div class="input-group input-group-solid mb-5 rounded-4">
                                ...
                                <input type="search" class="form-control form-control-solid text-dark" wire:model="search" placeholder="{{__('Search Clients, name, email, phone')}}" />
                            </div>
                        </div>
                    </div>
                    @if($clients->count() > 0)
                    <div class="table-responsive">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="{{route('user.manage', ['client' => $val->id, 'type' => 'details'])}}" class="btn btn-sm btn-light-info">Manage</a>
                                        <button id="kt_devices_{{$val->id}}_button" class="btn btn-sm btn-info">Devices</button>
                                        @if($val->status==0)
                        ...
                        @if($clients->total() > 0 && $clients->count() < $clients->total())<button wire:click="loadMore" wire:loading.remove class="btn btn-secondary btn-block">{{__('See more')}}</button>@endif
                    </div>
                    @else
    ...
    @foreach($clients as $val)
    <livewire:admin.users.edit-users :val=$val :admin=$admin :wire:key="'kt_devices_'. $val->id">
        @endforeach
</div>
@push('scripts')
<script>
...
<script>
    function initPhoneToggle() {
        ...
        @this.set('mobile_code', phoneInput.getSelectedCountryData().iso2);
