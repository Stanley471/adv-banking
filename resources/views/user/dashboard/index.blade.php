...
@section('content')
@livewire('balance', ['val' => $user->getFirstBalance()->getCurrency, 'user' => $user, 'settings' => $set, 'trx' => Str::random(16), 'type' => $type])
@stop
@section('script')
<script>
  "use strict"
  ...
  $('#reveal_balance').on('click', function() {
    $('#main_balance').text("{{currencyFormat(number_format($user->getBalance($user->getFirstBalance()->getCurrency->id)->amount,2)).' '.$currency->currency}}");
    $('#hide_balance').show();
    $('#reveal_balance').hide();
  });
  "use strict";
