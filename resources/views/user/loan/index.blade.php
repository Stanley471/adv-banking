...
<div class="toolbar" id="kt_toolbar">
    <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
        <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
            <h1 class="text-dark fw-bolder my-1 fs-2">{{__('Loan')}}</h1>
            <ul class="breadcrumb fw-semibold fs-base my-1 mb-6">
                ...
                <li class="breadcrumb-item text-dark">{{__('Loan')}}</li>
            </ul>
        </div>
    </div>
                ...
                <div class="col-xl-12">
                    <div class="card bg-transparent card-xl-stretch mb-5 mb-xl-8">
                        <div class="card-body p-0 pb-9">
                            @livewire('loan.index', ['user' => $user, 'settings' => $set])
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('#reveal_balance').on('click', function() {
        $('#main_balance').text("{{currencyFormat(number_format($user->pendingLoan('loan')->sum('payback'),2)).' '.$currency->currency}}");
        $('#hide_balance').show();
        $('#reveal_balance').hide();
    });
